package Smokeping::probes::FPingContinuousPlus;

=head1 301 Moved Permanently

This is a Smokeping probe module. Please use the command 

C<smokeping -man Smokeping::probes::FPingContinuousPlus>

to view the documentation or the command

C<smokeping -makepod Smokeping::probes::FPingContinuousPlus>
...
sub pod_hash {
      return {
	      name => <<DOC,
Smokeping::probes::FPingContinuousPlus - FPingContinuousPlus Probe for SmokePing
DOC
	      description => <<DOC,
Integrates FPingContinuousPlus as a probe into smokeping. The variable B<binary> must 
...
sub run_fping {
	...
	my $iptype = undef;
    $iptype = "-4" if $self->{properties}{interface} eq '4';
    $iptype = "-6" if $self->{properties}{interface} eq '6';
    ...
    push @params, "$iptype" if defined($iptype);
...
DOC
        ...
        iptype => {
			_example => '4',
			_doc => "The type of IP address to use for the ping.  This is useful if you have multiple IP addresses on the same name. Option '4' or '6', default is none.",
		},
	});
}
