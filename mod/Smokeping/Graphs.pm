...
sub get_fonts ($){
    ...
    my $defaultFont = $cfg->{Presentation}{defaultfont} || undef;
    my $defaultFontSize = $cfg->{Presentation}{defaultfontsize} || undef;
    my $titleFont = $cfg->{Presentation}{titlefont} || undef;
    my $titleFontSize = $cfg->{Presentation}{titlefontsize} || undef;
    my $axisFont = $cfg->{Presentation}{axisfont} || undef;
    my $axisFontSize = $cfg->{Presentation}{axisfontsize} || undef;
    my $unitFont = $cfg->{Presentation}{unitfont} || undef;
    my $unitFontSize = $cfg->{Presentation}{unitfontsize} || undef;
    my $legendFont = $cfg->{Presentation}{legendfont} || undef;
    my $legendFontSize = $cfg->{Presentation}{legendfontsize} || undef;
    my $watermarkFont = $cfg->{Presentation}{watermarkfont} || undef;
    my $watermarkFontSize = $cfg->{Presentation}{watermarkfontsize} || undef;

    if ($defaultFont && $defaultFontSize) {
        push(@cmd, '--font', "DEFAULT:$defaultFontSize:$defaultFont");
    } elsif ($defaultFont) {
        push(@cmd, '--font', "DEFAULT:0:$defaultFont");
    } elsif ($defaultFontSize) {
        push(@cmd, '--font', "DEFAULT:$defaultFontSize");
    };

    if ($titleFont && $titleFontSize) {
        push(@cmd, '--font', "TITLE:$titleFontSize:$titleFont");
    } elsif ($titleFont) {
        push(@cmd, '--font', "TITLE:0:$titleFont");
    } elsif ($titleFontSize) {
        push(@cmd, '--font', "TITLE:$titleFontSize");
    };

    if ($axisFont && $axisFontSize) {
        push(@cmd, '--font', "AXIS:$axisFontSize:$axisFont");
    } elsif ($axisFont) {
        push(@cmd, '--font', "AXIS:0:$axisFont");
    } elsif ($axisFontSize) {
        push(@cmd, '--font', "AXIS:$axisFontSize");
    };

    if ($unitFont && $unitFontSize) {
        push(@cmd, '--font', "UNIT:$unitFontSize:$unitFont");
    } elsif ($unitFont) {
        push(@cmd, '--font', "UNIT:0:$unitFont");
    } elsif ($unitFontSize) {
        push(@cmd, '--font', "UNIT:$unitFontSize");
    };

    if ($legendFont && $legendFontSize) {
        push(@cmd, '--font', "LEGEND:$legendFontSize:$legendFont");
    } elsif ($legendFont) {
        push(@cmd, '--font', "LEGEND:0:$legendFont");
    } elsif ($legendFontSize) {
        push(@cmd, '--font', "LEGEND:$legendFontSize");
    };

    if ($watermarkFont && $watermarkFontSize) {
        push(@cmd, '--font', "WATERMARK:$watermarkFontSize:$watermarkFont");
    } elsif ($watermarkFont) {
        push(@cmd, '--font', "WATERMARK:0:$watermarkFont");
    } elsif ($watermarkFontSize) {
        push(@cmd, '--font', "WATERMARK:$watermarkFontSize");
    };

    if (@cmd) { return @cmd[0..$#cmd] };
    return
}
...
#               "LINE1:dm2", # this is for kicking things down a bit
        ...
        push @task,
               ...
               Smokeping::Graphs::get_fonts($cfg),
                @G,
               "COMMENT:$ProbeDesc",
               "COMMENT:$date\\j";
